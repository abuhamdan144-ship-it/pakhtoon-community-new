...
export default function HeroLegendsCoverflow({ slides = defaultHeroLegends }) {
  ...
  const moveBy = useCallback((amount) => settle(Math.round(targetRef.current) + amount), [settle]);
  ...
  useEffect(() => {
    const interval = window.setInterval(() => moveBy(1), 5200);
    return () => {
