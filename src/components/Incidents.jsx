...
const fallbackIncidents = [
  { id: 1, type: 'Death', name: 'Rahimullah', desc: 'Passed away in Sohar hospital. Support needed for body repatriation.', date: '2026-08-21' },
  { id: 2, type: 'Injury', name: 'Abdullah Khan', desc: 'Workplace accident in Muscat. Needs medical funds.', date: '2026-08-15' },
];
...
export default function Incidents() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(query(collections.incidents), (snap) => {
            ...
            >
              ...
              <div className="bg-white/5 border border-white/10 p-6 rounded-xl backdrop-blur-sm hover:border-gold/30 transition-colors">
                <div className={`text-xs font-bold px-2 py-1 rounded inline-block mb-3 ${
                  incident.type === 'Death' ? 'bg-red/20 text-red-200' : incident.type === 'Injury' ? 'bg-orange-500/20 text-orange-200' : 'bg-blue-500/20 text-blue-200'
                }`}>
                  {incident.type}
                </div>
                ...
                <div className="flex items-center justify-between mt-4">
                  <span className="text-xs text-gray-500 font-mono">{incident.date || new Date().toLocaleDateString()}</span>
