  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] pt-20">
      ...
      {editingRecord && <Modal title={`${editingRecord.id ? 'Edit' : 'New'} ${{ donations: 'donation', incidents: 'incident', elections: 'election', ads: 'advertisement' }[editingRecord.type]}`} onClose={() => setEditingRecord(null)}><form onSubmit={saveOperationalRecord} className="grid gap-4 md:grid-cols-2">
        {editingRecord.type === 'donations' && <><Field label="Donor name" value={recordDraft.donor} onChange={(value) => setRecordDraft({ ...recordDraft, donor: value })} required /><Field label="Phone" value={recordDraft.phone} onChange={(value) => setRecordDraft({ ...recordDraft, phone: value })} /><Field label="Amount (OMR)" type="number" value={recordDraft.amount} onChange={(value) => setRecordDraft({ ...recordDraft, amount: value })} required /><Field label="Date" type="date" value={recordDraft.date} onChange={(value) => setRecordDraft({ ...recordDraft, date: value })} /><TextAreaField label="Description" value={recordDraft.description} onChange={(value) => setRecordDraft({ ...recordDraft, description: value })} /><ImageField label="Donation receipt / image" value={recordDraft.image} onClear={() => setRecordDraft({ ...recordDraft, image: '' })} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setRecordDraft({ ...recordDraft, image: await readImage(file) }); } catch (error) { setDataError(error.message); } }} /><StatusField value={recordDraft.status} options={['pending', 'approved', 'rejected']} onChange={(value) => setRecordDraft({ ...recordDraft, status: value })} /></>} 
        {editingRecord.type === 'incidents' && <><Field label="Reporter name" value={recordDraft.name} onChange={(value) => setRecordDraft({ ...recordDraft, name: value })} required /><Field label="Phone" value={recordDraft.phone} onChange={(value) => setRecordDraft({ ...recordDraft, phone: value })} /><Field label="Date" type="date" value={recordDraft.date} onChange={(value) => setRecordDraft({ ...recordDraft, date: value })} /><TextAreaField label="Incident description" value={recordDraft.description} onChange={(value) => setRecordDraft({ ...recordDraft, description: value })} required /><ImageField label="Incident evidence image" value={recordDraft.image} onClear={() => setRecordDraft({ ...recordDraft, image: '' })} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setRecordDraft({ ...recordDraft, image: await readImage(file) }); } catch (error) { setDataError(error.message); } }} /><StatusField value={recordDraft.status} options={['pending', 'published', 'closed', 'rejected']} onChange={(value) => setRecordDraft({ ...recordDraft, status: value })} /></>} 
        {editingRecord.type === 'elections' && <><Field label="Election title" value={recordDraft.title} onChange={(value) => setRecordDraft({ ...recordDraft, title: value })} required /><Field label="Date" type="date" value={recordDraft.date} onChange={(value) => setRecordDraft({ ...recordDraft, date: value })} /><TextAreaField label="Description" value={recordDraft.description} onChange={(value) => setRecordDraft({ ...recordDraft, description: value })} /><ImageField label="Election banner image" value={recordDraft.image} onClear={() => setRecordDraft({ ...recordDraft, image: '' })} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setRecordDraft({ ...recordDraft, image: await readImage(file) }); } catch (error) { setDataError(error.message); } }} /><StatusField value={recordDraft.status} options={['open', 'closed']} onChange={(value) => setRecordDraft({ ...recordDraft, status: value })} /></>} 
