...
export default function Admin() {
  ...
  const updateProfileStatus = async (type, record, status) => {
    setBusy(true);
    try {
      await updateDoc(doc(db, type === 'cabinet' ? 'cabinet' : 'news', record.id), { status, updatedAt: serverTimestamp(), updatedBy: user?.email || '' });
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] pt-20">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        <main className="mt-7 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-7 lg:p-9">
          ...
          <AnimatePresence mode="wait"><motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: .2 }}>
            ...
            {activeTab === 'Cabinet' && <ProfileGrid label="Cabinet members" records={cabinet} type="cabinet" onEdit={startProfileEdit} onDelete={deleteProfile} onStatus={updateProfileStatus} onAdd={startNewProfile} />}
            {activeTab === 'News' && <ProfileGrid label="News posts" records={news} type="news" onEdit={startProfileEdit} onDelete={deleteProfile} onStatus={updateProfileStatus} onAdd={startNewProfile} />}
      ...
      {editingRecord && <Modal title={`${editingRecord.id ? 'Edit' : 'New'} ${{ donations: 'donation', incidents: 'incident', elections: 'election', ads: 'advertisement' }[editingRecord.type]}`} onClose={() => setEditingRecord(null)}><form onSubmit={saveOperationalRecord} className="grid gap-4 md:grid-cols-2">
        ...
        {editingRecord.type === 'ads' && <><Field label="Ad title" value={recordDraft.title} onChange={(value) => setRecordDraft({ ...recordDraft, title: value })} required /><Field label="Destination link" value={recordDraft.link} onChange={(value) => setRecordDraft({ ...recordDraft, link: value })} /><TextAreaField label="Caption" value={recordDraft.description} onChange={(value) => setRecordDraft({ ...recordDraft, description: value })} /><StatusField value={recordDraft.status} options={['draft', 'published', 'archived']} onChange={(value) => setRecordDraft({ ...recordDraft, status: value })} /></>}
        <ImageField label="Category image" value={recordDraft.image} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setRecordDraft({ ...recordDraft, image: await readImage(file) }); } catch (error) { setDataError(error.message); } }} onClear={() => setRecordDraft({ ...recordDraft, image: '' })} />
...
function ProfileGrid({ label, records, type, onEdit, onDelete, onStatus, onAdd }) {
  return <section><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 className="font-bold text-gray-800">{label}</h2><div className="flex items-center gap-3"><span className="text-sm text-gray-500">{records.length} records</span><button type="button" onClick={() => onAdd(type)} className="inline-flex items-center gap-1 rounded-lg bg-forest-dark px-3 py-2 text-xs font-bold text-gold"><Plus size={14} />New {type === 'cabinet' ? 'cabinet member' : 'post'}</button></div></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{records.map((record) => { const isCabinet = type === 'cabinet'; const image = isCabinet ? record.photo : (record.image || record.photo); const title = isCabinet ? record.name : record.title; const currentStatus = record.status || (isCabinet ? 'published' : 'draft'); return <article key={record.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white"><div className="h-32 bg-forest-dark/5">{image && <img src={image} alt="" className="h-full w-full object-cover" />}</div><div className="p-5"><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-gray-800">{title || 'Untitled record'}</p><p className="mt-1 text-sm text-gray-500">{isCabinet ? record.position : record.summary || 'News post'}</p></div><StatusPill value={currentStatus} /></div><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={!onStatus} onClick={() => onStatus(type, record, currentStatus === 'published' ? 'draft' : 'published')} className="rounded-lg border border-green-200 px-2.5 py-2 text-xs font-bold text-green-700 disabled:opacity-50">{currentStatus === 'published' ? 'Unpublish' : 'Publish'}</button><button type="button" onClick={() => onEdit(type, record)} className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-xs font-bold text-gray-700"><Edit3 size={14} />Edit</button><button type="button" onClick={() => onDelete(type, record)} className="inline-flex items-center gap-1 rounded-lg border border-red-100 px-3 py-2 text-xs font-bold text-red-600"><Trash2 size={14} />Delete</button></div></div></article>; })}</div></section>;
}
