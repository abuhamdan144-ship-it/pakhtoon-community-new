...
const generateCardPin = () => {
  ...
  return String(100000 + ((values[0] || Math.floor(Math.random() * 900000)) % 900000));
};
...
export default function Admin() {
  ...
  const syncMemberCard = async (record) => {
    ...
    if (record.status === 'approved') {
      const cardPin = String(record.cardPin || generateCardPin()).replace(/\D/g, '').slice(0, 6).padStart(6, '0');
  ...
  const saveMember = async (event) => {
    ...
    try {
      if (creatingMember) {
        ...
        const cardPin = savedRecord.status === 'approved' ? String(savedRecord.cardPin || generateCardPin()).replace(/\D/g, '').slice(0, 6).padStart(6, '0') : savedRecord.cardPin || '';
