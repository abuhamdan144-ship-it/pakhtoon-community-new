...
export default function Admin() {
  ...
  const syncMemberCard = async (record) => {
    ...
    if (record.status === 'approved') {
      const cardPin = record.cardPin || deriveCardPin(record.phone);
  ...
  const issueMembershipId = async (member) => {
    ...
    const cardPin = String(member.cardPin || '1234').replace(/\D/g, '').slice(0, 6).padStart(4, '0');
    ...
    try {
      ...
      for (let offset = 0; offset < eligible.length; offset += 400) {
        ...
        eligible.slice(offset, offset + 400).forEach((member) => {
          ...
          const cardPin = member.cardPin || "1234";
  ...
  const saveMember = async (event) => {
    ...
    try {
      if (creatingMember) {
        ...
        const cardPin = savedRecord.cardPin || '1234';
