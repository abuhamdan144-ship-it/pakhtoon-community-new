...
exports.onMemberStatusUpdated = exports.refreshPublicMemberStats = exports.monthlyOpcBackup = exports.lookupMemberCardByPin = void 0;
...
const db = (0, firestore_2.getFirestore)(app, FIRESTORE_DATABASE_ID);
const storageBucket = (0, storage_1.getStorage)(app).bucket();
...
exports.lookupMemberCardByPin = (0, https_1.onCall)({ region: "us-central1" }, async (request) => {
    const lookup = String(request.data?.lookup || "").trim();
    const pin = String(request.data?.pin || "").replace(/\D/g, "");
    if (!lookup || !/^\d{4}$/.test(pin)) {
        throw new https_1.HttpsError("invalid-argument", "Enter the approved member name or mobile number and the last four phone digits.");
    }
    ...
    if (!card) {
        throw new https_1.HttpsError("not-found", "No approved membership card matched these details.");
    }
    return {
        approved: true,
...
function serialiseBackupValue(value) {
    ...
    if (Array.isArray(value))
        return value.map(serialiseBackupValue);
    if (value && typeof value === "object")
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, serialiseBackupValue(entry)]));
    return value;
}
exports.monthlyOpcBackup = (0, scheduler_1.onSchedule)({
    schedule: "0 2 1 * *",
    ...
    for (const collectionName of BACKUP_COLLECTIONS) {
        ...
        collections[collectionName] = snapshot.docs.map((entry) => ({ id: entry.id, data: serialiseBackupValue(entry.data()) }));
    }
    ...
    if (driveFolderId) {
        try {
            const googleAuth = new googleapis_1.google.auth.GoogleAuth({ scopes: ["https://www.googleapis.com/auth/drive.file"] });
            const drive = googleapis_1.google.drive({ version: "v3", auth: googleAuth });
            await drive.files.create({ requestBody: { name: `opc-firestore-${generatedAt.replace(/[:.]/g, "-")}.json`, mimeType: "application/json", parents: [driveFolderId] }, media: { mimeType: "application/json", body: stream_1.Readable.from([payload]) }, fields: "id,name" });
...
exports.refreshPublicMemberStats = (0, firestore_1.onDocumentWritten)({
    ...
    await db.doc("settings/publicStats").set({ totalMembers, approvedMembers, updatedAt: firestore_2.FieldValue.serverTimestamp() }, { merge: true });
    logger.info(`Updated public member stats: ${approvedMembers} approved of ${totalMembers} total.`);
});
exports.onMemberStatusUpdated = (0, firestore_1.onDocumentUpdated)({
    document: "members/{memberId}",
    database: FIRESTORE_DATABASE_ID,
}, async (event) => {
